...
export default function Analytics() {
  ...
  const { data: surveyData } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch("/api/analytics/survey-response-distribution", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-slate-900 mb-6">Detailed Analytics</h2>
      ...
      <Card className="p-4 mb-6">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <select className="px-3 py-2 border border-slate-300 rounded-lg">
            ...
            <option>Downtown Coffee Co.</option>
            <option>Bella Vista Restaurant</option>
          </select>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-200">
              {checkinData?.length === 0 ? (
                ...
                checkinData?.map((business: any, index: number) => (
                  <tr key={index} className="hover:bg-slate-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center">
                        <span className="text-slate-900 mr-2">N/A</span>
                      </div>
                    </td>
                  </tr>
                ))
